---
title: ガバナンス
sidebar:
  order: 7
---

> **非公式翻訳 (Unofficial translation).** これはデータパッケージ標準の有志による日本語訳です。
> 正典は英語原典 [datapackage.org](https://datapackage.org) 。未訳の箇所は英語の原文を参照してください。

データパッケージ標準は、さまざまな分野から集まった [Frictionless](https://frictionlessdata.io/) コミュニティのメンバーで構成されるワーキンググループによって統治されている。

ワーキンググループは非同期で作業を進めており、仕様に対するあらゆる変更にレビューモデルを用いている。また、ワーキンググループ向けの[月次の更新通話](https://forms.gle/UEqGnYKESqqw1LeW7)も開催しているが、これはもちろん必須ではなく、より広範なコミュニティにも開かれている。

## ワーキンググループ {#working-group}

ワーキンググループの構成は以下のとおりである。

### 拒否権を持つメンバー {#vetoing-members}

- **Evgeny Karev** ([Datist](https://datist.io/))
- **Peter Desmet** (Research Institute for Nature and Forest (INBO))

### 投票権を持つメンバー {#voting-members}

- **Phil Schumm** (CTDS - University of Chicago)
- **Kyle Husmann** (Pennsylvania State University)
- **Keith Hughitt** (National Institutes of Health)
- **Jakob Voß** (Verbundzentrale des GBV (VZG))
- **Ethan Welty** (World Glacier Monitoring Service (WGMS))
- **Paul Walsh** (Link Digital)
- **Pieter Huybrechts** (Research Institute for Nature and Forest (INBO))

### 諮問メンバー {#advisory-members}

- **Martin Durant** (Anaconda, inc.)
- **Adam Kariv** (The Public Knowledge Workshop)
- **Johan Richer** ([multi.coop](https://www.multi.coop/))
- **Steve Diggs** ([California Digital Library](https://cdlib.org/))

## 意思決定 {#decision-making}

仕様に対する変更案は、コミュニティまたはワーキンググループのメンバーが GitHub プルリクエストを用いて発議でき、ワーキンググループ内で合意に達した場合、すなわち全員が受け入れられる決定、あるいは少なくとも妥協点にたどり着いた場合に承認される。

ワーキンググループは、専用の GitHub プルリクエストで自身の見解を共有するよう求められる。より広範な議論が必要な場合、提案の議論は審議のために月次通話に引き上げられることがある。ワーキンググループには、提案された対応をレビューするための相応の時間が与えられる。

ワーキンググループのメンバーの少なくとも ⅔ が議論に参加して賛意を表明した場合に、合意に達したものとされ、課題はクローズされる。重大かつ明示的に表明された懸念がある場合、コアライブラリの出資者であるワーキンググループのメンバー(現時点では [Open Knowledge Foundation (OKFN)](https://okfn.org/)、[Research Institute for Nature and Forest (INBO)](https://www.vlaanderen.be/inbo/en-gb/homepage/)、[Datopian](https://www.datopian.com/))は、提案された対応に対して拒否権を行使できる。

OKFN のコミュニティマネージャーは、議論に参加しなかったワーキンググループのメンバーに連絡を取り、その意見も確実に捉えられるようにする。参加を促すリマインダーは慎重に扱われる。ワーキンググループのメンバーは、たとえば 2 週間に一度、投票が保留中の課題を知らせるメールを受け取るといった、穏やかで思いやりのある対応を期待してよい。これは、不必要な迷惑をかけることなくメンバーに情報を伝え続けることを目的としている。

仕様の技術的な保守に関する意思決定は、OKFN によって一元的に行われる。
