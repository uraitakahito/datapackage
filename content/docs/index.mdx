---
title: Data Package
template: splash
editUrl: false
hero:
  tagline: |
    Data Package is a <strong>standard</strong> consisting of a set of
    <strong>simple yet extensible specifications</strong>
    to describe datasets, data files and tabular data.
    It is a data definition language (DDL) and data API that facilitates
    <strong> findability, accessibility, interoperability, and reusability (FAIR)</strong> of data.
  image:
    file: ../../assets/hero.png
    alt: Data Packages Illustration
  actions:
    - text: Get Started
      # frontmatter の hero.actions は Starlight が base を補わないので相対で書く。
      # このページは base の直下 (/ または /datapackage/) なので相対で正しく解決する。
      link: overview/introduction/
      icon: right-arrow
      variant: primary
    - text: Use Cases
      link: overview/adoption/
      icon: rocket
      variant: secondary
    - text: View on GitHub
      link: https://github.com/frictionlessdata/datapackage
      icon: external
      variant: minimal
banner:
  content: |
    <p>
      The <strong>Data Package (v2)</strong> standard has been released on June 26, 2024.
      See the <a href="/blog/2024-06-26-v2-release/">announcement</a> and <a href="/overview/changelog/">changelog</a> for details.
    </p>
---

import About from "../../components/About.astro"
import Adoption from "../../components/Adoption.astro"
import ClickableCard from "../../components/ClickableCard.astro"
import { withBase } from "../../lib/base"
import { CardGrid, LinkCard, Card } from "@astrojs/starlight/components"

<h2>Standard</h2>

The Data Package standard is a comprehensive set of **specifications** that collectively define a framework for organizing, documenting, and sharing data in a structured and interoperable manner -- [EXPLORE THE STANDARD](/standard/data-package)

<CardGrid>
  <ClickableCard
    title="Data Package"
    icon="open-book"
    href={withBase("/standard/data-package")}
  >
    A simple container format to describe a coherent collection of data (a dataset),
    including its contributors, licenses, etc.
  </ClickableCard>
  <ClickableCard
    title="Data Resource"
    icon="rocket"
    href={withBase("/standard/data-resource")}
  >
    A simple format to describe a data resource such as an individual table or file,
    including its name, format, path, etc.
  </ClickableCard>
  <ClickableCard
    title="Table Dialect"
    icon="document"
    href={withBase("/standard/table-dialect")}
  >
    A simple format to describe the dialect of a tabular data file, including its
    delimiter, header rows, escape characters, etc.
  </ClickableCard>
  <ClickableCard
    title="Table Schema"
    icon="puzzle"
    href={withBase("/standard/table-schema")}
  >
    A simple format to describe tabular data, including field names, types, constraints,
    missing values, foreign keys, etc.
  </ClickableCard>
</CardGrid>

<div style="margin-bottom: 5em"></div>

<h2>Software</h2>

Data Package is backed by a suite of software tools supporting the standard. From the no-code visual tool **Open Data Editor** to low-level drivers for 10 programming languages -- [EXPLORE THE SOFTWARE](/overview/software)

<div style="margin-top: 10px; margin-bottom: 5em; padding: 5px; border: dashed 1px #ccc; border-radius: 15px">
  <a href="https://opendataeditor.okfn.org/">
    <img src={withBase("/ode.png")} style="border-radius: 15px" />
  </a>
</div>

<h2>Adoption</h2>

Data Package is used for a wide range of scenarios where this standardized data packaging format proves invaluable for efficient data management, sharing, and analysis -- [EXPLORE THE ADOPTION](/overview/adoption)

<Adoption />

<div style="margin-bottom: 5em"></div>

<h2>Documentation</h2>

Read the Data Package documentation to learn more about the project:

<CardGrid>
  <LinkCard title="Standard" href={withBase("/standard/data-package")} />
  <LinkCard title="Extensions" href={withBase("/extensions/camtrap-data-package")} />
  <LinkCard title="Recipes" href={withBase("/recipes/caching-of-resources")} />
  <LinkCard title="Guides" href={withBase("/guides/using-data-package")} />
</CardGrid>

<About title="Brought to you by">
  We are building a world open by design where all knowledge is accessible to everyone
  [Learn about Open Knowledge](https://okfn.org)
</About>
