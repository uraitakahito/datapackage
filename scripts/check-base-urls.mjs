 ...
 *
 * この fork は https://uraitakahito.github.io/datapackage/ に置かれるため、
 * astro.config.js が `base` を受け取る。Astro は **Markdown のリンク記法には
 ...
 * 誤検知を出した(astro.config.js のコメント参照)。出力を見れば、Astro が
 * 補ったあとの最終形をそのまま判定できるので誤検知が出ない。
 *
 *   node scripts/check-base-urls.mjs build /datapackage
 */
...
console.error(
  "\nMarkdown のリンク記法にするか、base を明示してください。" +
    ...
    "\n  astro.config.js: process.env.DP_BASE"
)
process.exit(1)
